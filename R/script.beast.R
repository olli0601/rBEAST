...
{
	stopifnot(c("CLU_ID", "TAXON_ID", "TAXON_NAME", "SEQ")%in%names(seq.select))
	...
	#	add new taxa 
	tmp 			<- subset(seq.select, select=TAXON_NAME)
	tmp[, BEASTlabel:=TAXON_NAME]
	setnames(tmp, 'TAXON_NAME', 'FASTASampleCode')
	...
	{
		tmp				<- tolower(do.call('rbind',strsplit(subset(seq.select, CLU_ID==clu)[, SEQ],'')))
		rownames(tmp)	<- subset(seq.select, CLU_ID==clu)[, TAXON_NAME]
		tmp				<- as.DNAbin(tmp)
